...
Project contract for Note — what the project is, how it's built, and how it runs. Written for anyone, human or AI, who needs to understand the project itself.
...
Note is a Windows PowerShell utility with a WPF interface. The repository is maintained as modular source, but the distributed artifact is one compiled PowerShell script.
...
- Release artifact: generated root `Note.ps1`.

### Repository Layout

- `Compile.ps1`: build script that creates `Note.ps1`.
...
- `Note.ps1`: ignored generated build artifact.

## Goals

- Provide a single-script Note that can be launched from PowerShell.
...
- `Note.ps1` is not hand-maintained.
...
`Compile.ps1` combines the repository sources into `Note.ps1` in this order:
...
6. Embed `tools/autounattend.xml` into `$NoteAutounattendXml`.
7. Append `scripts/main.ps1`.
8. Write the result to root `Note.ps1`.
...
- Note runs in PowerShell on Windows and uses WPF for the UI.
...
- `docs/Dockerfile` and `docs/docker-compose.yml` (service `Note-astro`) containerize the site's npm tooling; see AGENTS.md's Dependency Installs, Builds, And Dev Servers for why and how agents must use them instead of running npm on the host.

## Testing And CI

- `.\Compile.ps1` verifies the compiler can generate `Note.ps1`.
...
- The generated `Note.ps1` may appear locally after compile. It remains ignored build output (see root `.gitignore`) and must not be committed.

## Release Artifact

GitHub Actions is responsible for producing the release `Note.ps1` from repository sources. A release is considered valid only if the generated script came from the compile process, not from direct manual edits to `Note.ps1`.
